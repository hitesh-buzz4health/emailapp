        ...
        begin
          @start_sheet_no.upto(@total_no_of_sheets) do |sheet_num|
             ...
             if  ws[sheet_num].title.eql? "messages" 
                ...
                if  params[:facebook_email].nil? && (ws[sheet_num][4,1].length == 0 || ws[sheet_num][4,2].length == 0) 
              ...
              #saving cookies for the script.
              if File.exist?( user_email.to_s + '.txt')
              
                puts "FBGR: about to wait "  
                f = File.open(user_email.to_s + '.txt')
                s = f.gets
                d = s.split("@_@")
                d.each do |dd|
              ...
              File.open(user_email.to_s + '.txt', "wb") { |file| 
        ...
        if !user_email.nil?
             ...
             0.upto((ws.length) -1 ) do |sheet_num|
                ...
                if user_email.eql? ws[sheet_num][4,1]
                      ...
                      puts "FBGR: getting sheet no for " +ws[sheet_num][4,1]
                      puts "sheet no " + sheet_num.to_s
                end
